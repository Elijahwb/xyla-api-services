<?php
  ...
  if($connection->message == "CONNECTION SUCCESSFUL")
  {
    ...
    try
    {
      $names = explode(" ", trim($_POST['username']));
	  ...
	  //echo "<pre>";print_r($params);exit();
        $input_xml = '<?xml version="1.0" encoding="utf-8"?>
            <API3G>
                ...
                <Transaction>
                    ...
                    <PaymentAmount>'.$_POST['amount'].'</PaymentAmount>
                    <PaymentCurrency>UGX</PaymentCurrency>
                    <DefaultPayment>MO</DefaultPayment>
                    <CompanyRef>'.$_POST['email'].'</CompanyRef>
        ...
        if ($response === FALSE){
            ...
            array_push($tokenArray, 'Payment error in first_payment: Unable to connect to the payment gateway, please try again');
        }else{
            if ($xml->Result != '000') {
                array_push($tokenArray, 'Payment error code in first_payment: '.$xml->Result. ', '.$xml->ResultExplanation);
            }
            ...
            $statement->execute([$date,"Me",$_POST['occasion'],$_POST['instructions'],50000,intval($_POST['userid']),intval($_POST['celebrityid']),$xml->TransToken]);

            $statement = null;
            
            array_push($tokenArray, $xml->TransToken);
        }
